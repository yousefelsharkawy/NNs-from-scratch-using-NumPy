...
class BinaryClassifierNN:
    ...
    def train(self, X, Y , learning_rate, num_epochs, X_val = None, Y_val = None, batch_size = 64, lambd = 0, keep_prob = None, print_cost=True, optimizer = "gd", beta1 = 0.9, beta2 = 0.999, epsilon = 1e-8):
        ...
        for i in range(num_epochs):
            ...
            if print_cost and i % 1000 == 0:
                print ("Cost on the training set after epoch %i: %f" %(i, total_cost))
                if X_val is not None and Y_val is not None:
                    print ("Accuracy on the validation set after epoch %i: %f" %(i, self.accuracy(X_val, Y_val)))
...
#     my_model = BinaryClassifierNN(layer_dims=[train_X.shape[0], 5, 2, 1], activations=["relu", "relu", "sigmoid"])
